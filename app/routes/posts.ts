...
router.get("/user/:id", async (req, res) => {
  const userOne = await User.findOne({ where: { id: req.params["id"] } });
...
router.get("/category/:id", async (req, res) => {
  const categoryOne = await Category.findOne({ where: { id: req.params["id"] } });
...
router.get("/detail/:id", async (req, res) => {
  ...
  Comment.findAndCountAll({
    ...
    let judge: unknown = false; // Likeしたかの判定
    if (req.user) {
      judge = await Like.findOne({ where: { post_id: req.params["id"], user_id: req.user.id } });
    }
