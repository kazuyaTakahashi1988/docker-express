...
locals {
  ...
  cloud_build_source_bucket_name   = coalesce(var.cloud_build_source_bucket_name, "${var.project_id}-dockerexpress-cloudbuild-source")
...
resource "google_storage_bucket" "cloud_build_source" {
  name                        = local.cloud_build_source_bucket_name
  location                    = var.region
  uniform_bucket_level_access = true
...
resource "google_storage_bucket_iam_member" "compute_cloud_build_source_viewer" {
  bucket = google_storage_bucket.cloud_build_source.name
  role   = "roles/storage.objectViewer"
  member = "serviceAccount:${data.google_project.current.number}-compute@developer.gserviceaccount.com"
}

resource "google_storage_bucket_iam_member" "build_submitter_source_creator" {
  for_each = var.build_submitter_members
